...
var ysFilters = function () {
  ...
  this.blackAndWhite = function () {
    ...
    this.grayscale = function () {
      for (var index = 0; index < imageData.data.length; index += 4) {
        var v =
          0.3 * imageData.data[index] +
          0.59 * imageData.data[index + 1] +
          0.11 * imageData.data[index + 2];
        imageData.data[index] = v;
        imageData.data[index + 1] = v;
        imageData.data[index + 2] = v;
      }
    ...
    this.noise = function () {
      for (var index = 0; index < imageData.data.length; index += 4) {
        ...
        imageData.data[index + 1] =
          (imageData.data[index + 1] + this.random(0, 255)) / 2;
        imageData.data[index + 2] =
          (imageData.data[index + 2] + this.random(0, 255)) / 2;
      }
    ...
    this.sharpen = function () {
      return this.setMatrix([0, -1, 0, -1, 5, -1, 0, -1, 0]).convolute();
    };

    this.sobel = function () {
      return this.setMatrix([-1, -2, -1, 0, 0, 0, 1, 2, 1])
        .convolute()
        .setMatrix([-1, 0, 1, -2, 0, 2, -1, 0, 1])
        .convolute();
    };

    this.prewitt = function () {
      return this.setMatrix([-1, 0, 1, -1, 0, 1, -1, 0, 1])
        .convolute()
        .setMatrix([-1, -1, -1, 0, 0, 0, 1, 0, 1])
        .convolute();
    };
};
